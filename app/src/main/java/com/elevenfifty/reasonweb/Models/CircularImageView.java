...
public class CircularImageView extends ImageView
{
    ...
    private void init(Context context, AttributeSet attrs, int defStyle)
    {
        ...
        paintPadding = new Paint();
        paintPadding.setAntiAlias(true);
        ...
        // Check if border and/or padding is enabled
        hasBorder = attributes.getBoolean(R.styleable.CircularImageView_civ_border, false);
        hasPadding = attributes.getBoolean(R.styleable.CircularImageView_civ_padding, false);
        ...
        if(hasPadding) {
            int defaultPaddingSize = (int) (2 * context.getResources().getDisplayMetrics().density + 0.5f);
            setPaddingWidth(attributes.getDimensionPixelOffset(R.styleable.CircularImageView_civ_paddingWidth, defaultPaddingSize));
            setPaddingColor(attributes.getColor(R.styleable.CircularImageView_civ_paddingColor, Color.WHITE));
        }
    ...
    {
        if (paintPadding != null)
            paintPadding.setColor(paddingColor);
        this.invalidate();
    }
    ...
    public void onDraw(Canvas canvas)
    {
        ...
        if(hasSelector && isSelected) { // Draw the selector stroke & apply the selector filter, if applicable
            ...
            if(hasPadding) { // Draws a padding circle on top of the border circle if padding is enabled
                innerWidth = paddingWidth;
                outerWidth = innerWidth + borderWidth;
                center = (canvasSize - ((outerWidth) * 2)) / 2;

                paint.setColorFilter(null);
                canvas.drawCircle(center + outerWidth, center + outerWidth, center + outerWidth - 4.0f, paintBorder);
                canvas.drawCircle(center + outerWidth, center + outerWidth, center + innerWidth - 4.0f, paintPadding);
            } else {
                innerWidth = 0;
        ...
        canvas.drawCircle(center + outerWidth, center + outerWidth, center - 4.0f, paint);
    }
